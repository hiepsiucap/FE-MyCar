...
const MainDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const responsedata = await getRequest(
        `${import.meta.env.VITE_API_BACKEND}/api/cars/${id}`
      );
  ...
  const verifyDay = ({ start, end }) => {
    ...
    for (var a of DetailCar.days) {
      ...
      if (days.getTime() > start.getTime() && days.getTime() < end.getTime()) {
        setDateError(
          "Lịch bạn chọn đã có ngày xe bận, vui lòng chọn lại ngày nhận và trả xe"
        );
        return;
      }
    }
  ...
  const AddCartHandler = async () => {
    const dt = await getRequest(
      `${import.meta.env.VITE_API_BACKEND}/users/current`
    );
        ...
        <section className=" md:container mx-auto pt-8">
          ...
          >
            <motion.div
              ...
              className="bg-white rounded-xl py-6 px-16 w-full flex space-y-5 pb-24 flex-col items-center"
            >
              <h5 className=" font-manrope text-2xl font-bold pb-4 text-center">
                Chọn ngày thuê
              </h5>
              ...
              <div className=" flex w-full space-x-5 pb-5 rounded-b-2xl">
                <div className=" flex flex-col space-y-1  w-1/2">
                  <h5 className=" text-sm font-manrope text-gray-500">
                    Giờ nhận xe:
                  </h5>
                  <select
                    name=""
                    ...
                    className=" w-full border border-gray-400 rounded-md py-4"
                  >
                ...
                <div className=" flex flex-col space-y-1  w-1/2">
                  ...
                  <select
                    ...
                    className=" border border-gray-400 rounded-md w-full py-4"
                  >
                    {time.map((t) => {
                      return (
                        <option
              ...
              <div className=" flex w-full justify-between items-center 0 fixed bottom-0 px-5 py-5 bg-background">
                <div className=" flex flex-col font-manrope font-bold">
                  <p>
                    {" "}
                    {startHour + ", "} {startDate?.getDate() < 10 && 0}
                    {startDate?.getDate()}/{startDate?.getDate() < 10 && 0}
                    {startDate?.getMonth() + 1} - {EndHour + ", "}{" "}
                    {endDate?.getDate() < 10 && 0}
                    {endDate?.getDate() || 0}/{endDate?.getDate() < 10 && 0}
                    {endDate?.getMonth() + 1 || 0}
                  </p>
                  <p> Số ngày thuê: {tinhSoNgay(startDate, endDate) || 0} </p>
                </div>
                <button
                  className=" font-bold w-1/3 font-manrope font-lg py-2 px-4 bg-primary text-white rounded-md"
                  onClick={closeModal}
                >
                  Xác nhận
                </button>
          ...
          <div className=" flex space-x-10 pt-10">
              ...
              <div>
                ...
                <div className=" flex justify-between border-b-2 pb-6 border-gray-200">
                  <div className=" flex space-x-2 items-center">
                    ...
                    <p className=" font-manrope text-gray-500">
                      Thành phố {data.province}
                    </p>
                  </div>
                  <div className=" flex space-x-2 items-center">
                    <button className=" font-manrope font-bold text-lg text-gray-800">
                      Chia sẻ
                    </button>
                    <img
                      src={chiase}
                ...
                <div className=" flex flex-col space-y-8 py-8">
                  <div className=" flex flex-col space-y-2">
                    <h5 className=" font-bold font-manrope text-xl">
                      Đặc điểm
                    </h5>
                    ...
                    <div className=" flex justify-between pt-3">
                      ...
                      <div className="flex justify-start space-x-4 ">
                        ...
                        <div>
                          ...
                          <p className=" text-xl font-medium ">
                            {data.gear === "Tu dong" ? "Tự động" : "Số sàn"}
                          </p>
                        </div>
                      </div>
                      <div className="flex justify-start space-x-4 ">
                  ...
                  <div className=" flex flex-col space-y-2">
                    ...
                    <div className="pt-1 px-1 rounded-sm bg-primary w-12"></div>
                    <div className=" pt-3 leading-relaxed font-manrope">
                      {data.description}
                    </div>
                  </div>
                  <div className=" flex flex-col space-y-2">
                    ...
                    <div className=" pt-3 leading-relaxed font-manrope">
                      ...
                      <ul>
                        ...
                        <li>
                          ...
                          hàng vui lòng vệ sinh xe sạch sẽ hoặc gửi phụ thu phí
                          vệ sinh xe.
                        </li>
                      </ul>
                      <p>
                        Trân trọng cảm ơn, chúc quý khách hàng có những chuyến
                        đi tuyệt vời!
                      </p>
                    </div>
                  </div>
                    ...
                    {data.reviews.map((r) => {
                      return (
                        <motion.div
                          ...
                          transition={{ duration: 0.5 }}
                          key={r.name}
                          className=" flex flex-col px-6 py-4 space-y-3 border rounded-lg border-slate-300 "
                        >
                          <div className=" flex justify-between">
                            <div className=" flex space-x-4">
                              ...
                              />
                              <div className=" flex flex-col space-y-2">
                                <h2 className=" font-medium text-xl font-manrope">
                                  {r.name}
                                </h2>
                                <div className=" flex ">
                                  {renderStars(r.rate)}
            ...
            <div className=" shadow-md bg-slate-50 flex flex-col space-y-5 items-center p-5 w-1/3 h-min">
              <h1 className=" text-3xl font-manrope text-primary font-bold">
                {formatPrice(data.cost)}/1 Ngày
              </h1>
              <div className=" w-full">
                <div className=" flex w-full">
                  <button
                    className={
                      DateError
                        ? "bg-white border border-red-400 rounded-l-md p-3 w-1/2"
                        : "bg-white border rounded-l-md p-3 w-1/2"
                    }
                    onClick={openModal}
                  >
                    <p className="text-sm font-manrope text-gray-600 text-start">
                      Nhận xe{" "}
                    </p>
                    <p className=" font-manrope tracking-wide font-medium w-full text-start">
                  ...
                  <button
                    className={
                      DateError
                        ? "bg-white border border-red-400 rounded-r-md p-3  w-1/2"
                        : "bg-white border rounded-r-md p-3  w-1/2"
                    }
                    onClick={openModal}
                  >
                    <p className="text-sm font-manrope text-gray-600 text-start">
                    ...
                    <p className=" font-manrope tracking-wide font-medium text-start">
                      {endDate?.getDate() < 10 && 0}
                      {endDate?.getDate()}/{startDate?.getMonth() + 1 < 10 && 0}
                      {endDate?.getMonth() + 1}/{endDate?.getFullYear()} &nbsp;{" "}
                      {EndHour}
                    </p>
                  </button>
                </div>
                <p className=" text-red-500 text-xs italic text-start pt-1">
                  {DateError}
                </p>
              </div>
              <button className="bg-white border flex flex-col space-y-2 rounded-md p-5 w-full">
                ...
                <p className="text-sm font-manrope text-gray-400 text-start">
              ...
              <div className=" flex w-full justify-between">
                ...
                <div className=" flex space-x-2 items-end">
                  <p className=" text-sm font-manrope text-gray-600 pb-1 ">
                    {tinhSoNgay(startDate, endDate)}*{formatPrice(data.cost)}=
                  </p>
              ...
              {DateError || totalerror ? (
                <button
                  to="/booking"
                  disabled
                  className=" bg-primary opacity-60 rounded-md p-4 w-1/2 mx-auto font-manrope text-lg font-bold text-white"
                ...
                <button
                  to="/booking"
                  className=" bg-primary rounded-md p-4 w-1/2 mx-auto font-manrope text-lg font-bold text-white"
                  onClick={AddCartHandler}
                >
                  Thanh toán
                </button>
          ...
          <div className=" py-16">
            <h5 className=" text-4xl font-bold font-manrope text-center pb-12">
              Xe tương tự
            </h5>
            <CarSlick></CarSlick>
          </div>
