...
const Revenue = () => {
  const Number = ({ n }) => {
    ...
    return (
      <animated.div className=" font-manrope font-bold text-2xl text-primary">
  ...
  useEffect(() => {
    const getData = async () => {
      const response = await getRequest(
        `${import.meta.env.VITE_API_BACKEND}/users/revenue`
      );
      if (response.error) {
        return;
      }
  ...
  return (
    <>
      <motion.section
        ...
        className=" w-full"
      >
        <h5 className=" font-manrope text-3xl  font-bold text-start pb-10">
          Quản lý tài chính
        </h5>
        <div className=" flex justify-center space-x-10 w-full">
          <div className=" bg-white w-1/4 flex p-5 space-x-5  rounded-lg border border-primary border-opacity-50 ">
            ...
            <div className=" flex flex-col  items-start">
              <div className=" font-manrope font-medium text-gray-700 text-sm">
                Tổng số xe:{" "}
              </div>
        ...
        <div className=" flex flex-col items-center space-y-8 pt-16">
          <h5 className=" font-manrope text-2xl font-bold">
            Doanh thu theo tháng (triệu đồng)
          </h5>
          {revenue.monthrent && (
            <ApexChart monthrevenue={revenue.monthrevenue}></ApexChart>
          )}
          <h5 className=" font-manrope text-2xl font-bold pt-4">
            Lượt đặt xe theo tháng (lần)
          </h5>
          {revenue.monthrent && (
            <LineApexChart monthrent={revenue.monthrent}></LineApexChart>
          )}
          <h5 className=" font-manrope text-2xl font-bold pt-4">
            Top xe có doanh thu cao nhất
          </h5>
          {revenue.topcar && revenue.toprentcar && (
            <BarChart
              topcar={revenue.topcar}
              toprentcar={revenue.toprentcar}
            ></BarChart>
          )}
        </div>
      </motion.section>
