  ...
  useEffect(() => {
    const getData = async () => {
      const responsedata = await getRequest(
        `${import.meta.env.VITE_API_BACKEND}/api/cars/mycar`
      );
  ...
  return (
    ...
    >
      <div className=" justify-between flex items-center pb-5">
        ...
        <Link
          to="/registercar"
          className="  px-5 py-3 font-manrope font-medium text-white  bg-primary rounded-lg"
        >
          Đăng kí xe mới
        </Link>
      </div>
            ...
            <>
              <div className=" p-5 pb-10 flex bg-white w-full rounded-lg shadow-sm">
                ...
                <div className=" flex flex-col justify-center items-center w-1/4 space-y-2.5">
                  <h5 className=" text-2xl font-manrope font-bold text-primary pb-3">
                    {!car.status && "Đang xét duyệt"}{" "}
                    {car.status == "active" && "Đang cho thuê"}{" "}
                    <span className=" text-red-600">
                      {car.status == "pause" && "Dừng cho thuê"}{" "}
                      {car.status == "pending" && "Đăng kí thất bại"}
                    </span>
                  </h5>
                  <Link
                    to="/updatecar/1"
                    className=" p-2.5 px-5 bg-primary  rounded-md font-manrope text-white text-sm font-medium"
                  >
                    cập nhật thông tin{" "}
                  </Link>
                  {car.status == "pause" && (
                    <button
                      id={car.carId}
                      className=" p-2 px-6 border border-primary rounded-md font-manrope  text-sm font-medium"
                  ...
                  {car.status == "active" && (
                    <button
                      id={car.carId}
                      className=" p-2 px-8 border border-primary rounded-md font-manrope  text-sm font-medium"
                  ...
                  <Link
                    to={`/calendar/${car.carId}`}
                    className=" p-2 px-9 border border-primary rounded-md font-manrope  text-sm font-medium"
                  >
                    Lịch cho thuê
                  </Link>
                </div>
