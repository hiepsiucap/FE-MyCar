...
const LicensePage = () => {
  ...
  const onCheckHandler = async (e) => {
    const response = await postRequest(
      `${import.meta.env.VITE_API_BACKEND}/users/licensecheck?id=${
        e.target.id
      }&check=true`,
      {}
    );
  ...
  useEffect(() => {
    const getData = async () => {
      const responsedata = await getRequest(
        `${import.meta.env.VITE_API_BACKEND}/users/staff/license`
      );
              ...
              <div className=" p-5 pb-10 flex bg-white w-full rounded-lg shadow-sm">
                <div className=" w-3/4 flex items-center space-x-5 border-r-2 border-primary border-opacity-40 ">
                  ...
                  <div className=" flex flex-col space-y-2 w-full">
                    <h5 className=" font-manrope font-bold text-start text-2xl pb-2">
                      {" "}
                    ...
                    <div className=" flex space-x-1">
                      <p className=" text-gray-500 text-sm font-manrope pr-2"></p>
                      <p className=" text-gray-500 text-sm font-manrope pr-2">
                        Ngày tạo: {formatDate(user.create_date)}
                      </p>
                    </div>
                    <div className=" flex space-x-1">
                      <p className=" text-gray-500 text-sm font-manrope pr-2"></p>
                      <p className="  text-sm font-manrope pr-2 text-red-500">
                        Trạng thái:{" "}
                        {user.driverLicenseCheck == null && "Chưa xét duyệt"}
                      </p>
                    </div>
                  </div>
                </div>
                <div className=" flex flex-col justify-center items-center w-1/4 space-y-3">
                  <button
                    className=" p-2 px-5 w-40 bg-primary rounded-md font-manrope text-white text-sm font-medium"
                    onClick={() => {
                      changeurl(user.driverLicense);
                      openModal();
                    }}
                  >
                    Xem bằng lái
                  ...
                  <button
                    id={user.userId}
                    className=" p-2 px-8 border w-40 border-primary rounded-md font-manrope  text-sm font-medium"
