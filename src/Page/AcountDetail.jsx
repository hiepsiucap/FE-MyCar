...
const AccountDetail = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      const responedt = await getRequest(
        `${import.meta.env.VITE_API_BACKEND}/users/numofrentals`
      );
    ...
    const ChangeAva = async () => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_BACKEND}/users/avatar`,
          {
            method: "POST",
            withCredntials: true,
            credentials: "include",
  ...
  const ChangePasswordSubmit = async (e) => {
    ...
    const response = await patchRequest(
      `${import.meta.env.VITE_API_BACKEND}/users/changepassword`,
      password
    );
    if (response.error) {
      return Swal.fire({
  ...
  const ConfirmHandler = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await fetch(
            `${import.meta.env.VITE_API_BACKEND}/users/license`,
            {
              method: "POST",
              withCredntials: true,
              credentials: "include",
    ...
    <div className=" flex flex-col space-y-5 w-full">
      ...
      >
        <motion.div
          ...
          className="bg-white rounded-xl py-6 mx-auto w-3/4 flex space-y-2 pb-12 flex-col items-center"
        >
          <p className=" font-bold text-2xl text-center font-manrope">
            Đổi mật khẩu
          </p>
          <form
            onSubmit={ChangePasswordSubmit}
            className=" flex-col space-y-3 w-3/4 items-start py-8"
          >
          ...
          <button
            className=" font-bold w-1/2 font-manrope font-lg py-3 px-4 mx-auto bg-primary text-white rounded-md"
      ...
      <motion.section
        ...
        className=" bg-white rounded-lg px-8 py-6 w-full"
      >
        <div className=" flex justify-between items-center">
          ...
          <div className=" flex items-end space-x-1 p-4 border-2 border-primary rounded-lg ">
            <svg
            ...
            <h1 className=" pb-1  font-manrope text-sm text-gray-500">
              chuyến
            </h1>
          </div>
        </div>
        <div className=" flex space-x-16 pt-12 items-center">
          <div className=" flex flex-col space-y-5 items-center  w-1/3">
            ...
            <h5 className=" text-xl font-manrope font-medium">
              {userInfo.first_name} {userInfo.last_name}
            </h5>
            <h5 className=" text-sm text-gray-400  font-manrope font-medium">
              Tham gia: {formatDate(userInfo.create_date)}
            </h5>
            {imageloading ? (
              <button
                disabled
                className=" w-28 h-10 bg-primary font-manrope text-white text-sm  rounded-lg flex justify-center items-center bg-opacity-80"
              ...
              <button
                className=" w-28 h-10 bg-primary font-manrope text-white text-sm  rounded-lg"
                onClick={onChooseFile2}
              >
                Thay đổi ảnh
              </button>
          ...
          >
            <div className=" flex flex-col space-y-1 items-start">
              ...
              <input
                ...
                name="fname"
                className="px-1 py-2.5 border border-slate-300 w-5/6 rounded-lg"
              />
            </div>
            <div className=" flex flex-col space-y-1 items-start">
              <label
            ...
            <div className=" py-5">
              <input
                ...
                className=" hidden"
              ></input>
              <button className=" py-4 px-6 w-1/3 text-white bg-primary font-manrope font-bold rounded-lg">
                Thay đổi
              </button>
            </div>
          </form>
      ...
      >
        <div className=" flex justify-between items-center">
            ...
            {userInfo.driverLicenseCheck == null && (
              <>
                <p className="px-2 py-1 rounded-full space-x-1 flex bg-red-200  -translate-y-1">
                  ...
                  <p className=" text-xs text-gray-800 font-manrope ">
                    Chưa xác thực
                  </p>
                </p>
              </>
          ...
          <div>
            ...
            <button
              className=" font-manrope text-sm px-4 py-2 bg-primary rounded-lg text-white"
        ...
        <div className=" flex space-x-10 pt-8 justify-center ">
          <div className=" w-2/3 h-full flex-col py-6">
            ...
            {!imageloading1 ? (
              <button
                className=" relative w-full flex flex-col h-full px-24 py-36 border border-dashed border-primary rounded-lg justify-center items-center"
              ...
              >
                {selectedFile && (
                  <img
                    ...
                    src={
                      selectedFile !== null && URL.createObjectURL(selectedFile)
                    }
                    alt=""
                  />
                )}
                {!selectedFile && userInfo.driverLicense && (
                  <img
                    className=" absolute w-full h-full scale-95 rounded-md"
                    src={userInfo.driverLicense}
                    alt=""
                  />
                )}
                <svg
            ...
            <div
              className={
                selectedFile?.name
                  ? " p-2 rounded-full bg-blue-300 text-sm text-black font-bold font-manrope w-2/3 mx-auto overflow-hidden"
                  : " p-2 rounded-full bg-blue-300 text-sm text-white font-manrope w-2/3 mx-auto"
              }
            >
