  ...
  useEffect(() => {
    const getData = async () => {
      const responsedata = await getRequest(
        `${import.meta.env.VITE_API_BACKEND}/users/cars/${id}/history`
      );
                ...
                cardata.map((car) => {
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">{car.rentalDate}</td>
                      <td className="px-6 py-4">{car.totalDay}</td>
                      <td className="px-6 py-4">
                        {formatPrice(car.totalCost)}
                      </td>

                      <td className="px-6 py-4">
                        {car.rentalStatus === "completed" && "Hoàn thành"}{" "}
                        {car.rentalStatus === "confirmed" && "Đang cho thuê"}{" "}
                        {car.rentalStatus === "pause" && "Ngưng cho thuê"}
                      </td>
                      <td className="px-6 py-4">
