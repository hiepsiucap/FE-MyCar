...
const Booking = () => {
  ...
  const { rentalinfo, carinfo } = useSelector((state) => state.rental);
  ...
  const SubmitHanlder = async () => {
    ...
    if (response.error) {
      ...
      const responsedata = await getRequest(
        `${import.meta.env.VITE_API_BACKEND}/api/cars/${carId}`
      );
  ...
  if (!pickUpDate || !dropOffDate || !userInfo)
    return (
      <h1 className=" font-bold text-primary text-2xl py-24">
  ...
  return (
    <>
      ...
      >
        <Link
          to="/"
          className=" flex space-x-2 w-1/2 mx-auto"
        >
          <svg
        ...
        <div className=" w-1/2 mx-auto bg-white rounded-md shadow-md p-5">
          ...
          <p className=" text-xs text-gray-500 pt-2 ">
            Vui lòng để lại thông tin liên lạc. Chúng tôi sẽ liên hệ bạn sớm
            nhất
          </p>
          ...
          <div className=" flex flex-col space-y-6 pt-6">
            <div className="  text-lg text-start font-bold font-manrope">
              Thông tin đơn hàng
            </div>
            <div className=" flex space-x-2">
              ...
              <div className=" flex flex-col text-start space-y-1">
                ...
                <p className=" font-manrope  font-bold">
                  {pickUpHours + ", "} {pickUpDate?.getDate() < 10 && 0}
                  {pickUpDate?.getDate()}/{pickUpDate?.getMonth() < 10 && 0}
                  {pickUpDate?.getMonth() + 1}/
                  {pickUpDate?.getFullYear() < 10 && 0}
                  {pickUpDate?.getFullYear()} - {dropOffHours + ", "}{" "}
                  {dropOffDate?.getDate() < 10 && 0}
                  {dropOffDate?.getDate() || 0}/
                  {dropOffDate?.getMonth() < 10 && 0}
                  {dropOffDate?.getMonth() + 1 || 0}/
          ...
          <div className=" w-full flex pt-6">
            <button
              className=" font-manrope bg-primary text-lg w-full text-white p-2 font-bold rounded-md "
