...
export const fetchCars = createAsyncThunk(
  "listcar/fetchCars",
  async ({ rangeprice, brand, fuel, province, district, type, page }) => {
    let params = {};
    console.log(params);
    ...
    if (rangeprice.length == 2) {
      if (rangeprice[0] !== null)
        params = { ...params, minprice: rangeprice[0] * 100000 };
      if (rangeprice[1] !== null)
        params = { ...params, maxprice: rangeprice[1] * 100000 };
    }
    if (page != 0) {
    ...
    const response = await axios.get(
      `${import.meta.env.VITE_API_BACKEND}/api/cars/conditions`,
      { params }
    );
...
const listcarSlice = createSlice({
  ...
  reducers: {
    ...
    resetfilter: (state) => {
      ...
      (state.brand = null), (state.fuel = []), (state.province = null);
      (state.district = null), (state.type = []), (state.page = 0);
    },
...
export const {
  ...
  changefuel,
  changetype,
  changedistrictlocation,
  changeprovincelocation,
  changelocation,
  resetfilter,
  changerangeprices,
  changepage,
} = listcarSlice.actions;
